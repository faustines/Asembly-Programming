...
_start:
	...
	*/
	...
	outer_for_loop:
		cmpl word1_len, %ecx #ecx - word1_len
		jg end_outer #ecx - word1_len > 0 (NEGATION)
			...
			inner_for_loop:
				cmpl word2_len, %edx #edx - word2_len
				jg end_inner ##edx - word2_len > 0 (NEGATION)
				#word1 = string1
				#word2 = string2
				...
				if:
					movb string1 - 1*ws(, %ecx, ws), %dl
					movb string2 - 1*ws(, %edx, ws), %al
					cmpb %al, %dl
					jnz else # != 0
